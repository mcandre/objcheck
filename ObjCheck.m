...
- (id) performSelector: (const SEL) sel withArgs: (NSArray *) args {
  ...
  for (i = 0; i < [args count]; i++) {
    id a = args[i];
    [inv setArgument: &a atIndex: (NSInteger)(2 +
        i)];  // 0 is target, 1 i cmd-selector
  }
...
+ (NSString *) genString {
  NSArray *arr = [self genArray: ^ () {
    return (id) [ObjCheck genChar];
  }];
...
+ (BOOL) forAll: (const id) target withProperty: (const SEL) property
  ...
  for (i = 0; i < 100; i++) {
    ...
    const NSNumber *propertyHolds = [target performSelector: property withArgs:
                                     values];

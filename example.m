...
int main() {
  ...
  [gs addObject: ^ () {
    return [ObjCheck genNum];
  }];

  // Are all integers even?
  [ObjCheck forAll: [Example class] withProperty: @selector(
     isEven:) withGenerators: gs];

  NSMutableArray *gs2 = [NSMutableArray array];
  [gs2 addObject: ^ () {
    return [Example genEven];
  }];

  // Are all even integers even?
  [ObjCheck forAll: [Example class] withProperty: @selector(
     isEven:) withGenerators: gs2];

  NSMutableArray *gs3 = [NSMutableArray array];
  [gs3 addObject: ^ () {
    return [ObjCheck genString];
  }];

  // Are all strings reversible?
  [ObjCheck forAll: [Example class] withProperty: @selector(
     reversible:) withGenerators: gs3];
